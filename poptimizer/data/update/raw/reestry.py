...
class Service:
    ...
    @DataUpdateError.suppress_errors(logging.getLogger("Reestry"))
    ...
    async def _download(self, status_row: status.Status) -> str:
        ...
        async with self._session.get(f"{_URL}{ticker}") as resp:
            if not resp.ok:
                raise DataUpdateError(f"{status_row.ticker} bad respond status {resp.reason}")
...
def _validate_header(row: html.HtmlElement, data_col: int) -> None:
    ...
    if not any(word in header for word in share_type):
        raise DataUpdateError(f"wrong dividends table header {header}")
...
def _parse_date(date_raw: str) -> datetime:
    if not (date_re := _RE_DATE.search(date_raw)):
        raise DataUpdateError(f"can't parse date {date_raw}")
...
def _parse_div(div_raw: str) -> tuple[float, domain.Currency]:
    if not (div_re := _RE_DIV.search(div_raw)):
        raise DataUpdateError(f"can't parse dividends {div_raw}")
    ...
    match div_re[2]:
        ...
        case _:
            raise DataUpdateError(f"unknown currency {div_re[2]}")
