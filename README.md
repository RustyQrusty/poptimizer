...
Install
---------
- open shell in project root directory with `docker-compose.yml` file and run commands
    ```
    docker-compose build

    docker-compose up -d
    ```
- attach VS Code to server docker container default name is `poptimizer-server`
- open shell in `home/root/src` directory container and run command
    ```
    python3 poptimizer --help
    ``` 
...
Update dividents
---------
- connect MongoDB Compass to database by URI `mongodb://127.0.0.1:27017`
- update dividents for new or current tickers
